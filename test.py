...
def run(args, mesh, views, FLAMEServer, deformer_net, shader, renderer, device, channels_gbuffer, lgt):
    ...
    # only deformed vertices positions no vertex index, so it is very hard to save as obj
    ...
    #return the deformed mesh as well
    # deformed_mesh = Mesh(deformed_vertices, mesh.indices);
    return rgb_pred, gbuffers, cbuffers # deformed_mesh
...
def quantitative_eval(args, mesh, dataloader_validate, FLAMEServer, deformer_net, shader, renderer, device, channels_gbuffer,
    ...
    for it, views_subset in enumerate(dataloader_validate):
        ...
        if save_each:   #save rgb and normals
            save_individual_img(rgb_pred, views_subset, gbuffer["normal"], gbuffer["mask"], cbuffer, images_eval_save_path)
            #save_individual_obj(deformed_mesh, views_subset, images_save_path);   # use the same image save path
...
if __name__ == '__main__':
    ...
    # # ==============================================================================================  
    print("Eval the quantitative part for RGB only!");
