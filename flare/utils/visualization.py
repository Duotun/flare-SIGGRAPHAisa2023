...
from flare.utils import write_mesh
...
def save_individual_img(rgb_pred, views, normals, gbuffer_mask, buffers, images_save_path):
    ...
    #print("len of gbuffer_mask: ", len(gbuffer_mask));
    for i in range(len(gbuffer_mask)):
        mask = gbuffer_mask[i].cpu()
        id = int(views["frame_name"][i])
...
def save_individual_obj(mesh, views, mesh_save_path):
    Path(mesh_save_path / "mesh").mkdir(parents=True, exist_ok=True)
    id = int(views["frame_name"][0])   # 0 is ok, no need to go through RGBA
    mesh_path = mesh_save_path / "rgb" / f'{id:05d}.obj'
